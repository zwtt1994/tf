...
# RNN
rnn_cell = tf.contrib.rnn.BasicRNNCell(num_units=CELL_SIZE)     # cell size 32
...
outs2D = tf.reshape(outputs, [-1, CELL_SIZE])    # [bacth*10,32]              # reshape 3D output to 2D for fully connected layer
net_outs2D = tf.layers.dense(outs2D, INPUT_SIZE) # [bacth*10,1]
outs = tf.reshape(net_outs2D, [-1, TIME_STEP, INPUT_SIZE]) # [bacth,10,1]         # reshape back to 3D
